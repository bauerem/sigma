...
class chatBot():
    ...
    def rule(self):
        ...
        self.rag()
    def rag(self):
        retrieved_corpus=self.retriever(last=self.last_utterance,history=self.history)
        history="\n".join(b+": "+t for b,t in self.history)
        ret="\n\n".join(str(i)+". "+r for i,r in enumerate(retrieved_corpus))
        res=call_gpt4_api(rag_prompt.replace("<conversation>",history).replace("<snippets>",ret).replace("<language>",self.language))
